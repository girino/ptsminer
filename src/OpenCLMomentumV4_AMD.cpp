...
OpenCLMomentumV4_AMD::OpenCLMomentumV4_AMD(int _HASH_BITS, int _device_num) {
	...
	// compiles
	fprintf(stdout, "Starting OpenCLMomentum V4 AMD Optimized\n");
	...
	program_filenames.push_back("opencl/OpenCLMomentumV4_AMD.cl");
	...
	hashes_wgsize = kernel_calculate_all_hashes->getWorkGroupSize(main.getDevice(device_num));
	...
	kernel_calculate_all_hashes->addLocalArg(sizeof(uint64_t)*hashes_wgsize);
	kernel_calculate_all_hashes->addLocalArg(sizeof(uint64_t)*26*hashes_wgsize);
	kernel_calculate_all_hashes->addLocalArg(sizeof(uint8_t)*36*hashes_wgsize);
...
OpenCLMomentumV4_AMD::~OpenCLMomentumV4_AMD() {
	// destroy
	delete hashes;
	delete hash_table;
	delete (collisions);
...
void OpenCLMomentumV4_AMD::find_collisions(uint8_t* message, collision_struct* out_buff, size_t* out_count) {
...
//	queue->finish();
	...
	// use precalculated wg size based on local mem size
...
int OpenCLMomentumV4_AMD::getCollisionCeiling() {
	return (1<<8);
}
